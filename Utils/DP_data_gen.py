...
class Task_Generator(object):
    ...
    def get_data_loader(self, prm, meta_split='meta_train', limit_train_samples=None):
        ...
        if self.data_source == 'MNIST':
            train_dataset, data_prior_dataset, test_dataset = load_MNIST(final_input_trans, target_trans, prm)
        ...
        data_prior_loader = data_utils.DataLoader(data_prior_dataset, batch_size=prm.data_prior_batch_size, shuffle=True, **kwargs)
        ...
        data_loader = {'train': train_loader, 'test': test_loader, 'data_prior': data_prior_loader,
                       'data_prior_ratio': prm.data_prior_ratio, 'n_data_prior_samples': n_data_prior_samples,
...
def load_MNIST(final_input_trans, target_trans, prm):
    ...
    data_prior_size = int(prm.data_prior_ratio * len(train_dataset))
    train_size = len(train_dataset) - data_prior_size
    train_dataset, data_prior_dataset = torch.utils.data.random_split(train_dataset, [train_size, data_prior_size])
    ...
    n_data_prior_samples = len(data_prior_dataset)
    n_test_samples = len(test_dataset)

    return train_dataset, data_prior_dataset, test_dataset
