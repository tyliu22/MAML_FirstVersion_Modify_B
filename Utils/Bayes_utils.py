    ...
    if n_train_tasks == 0:
        ...
        if prm.complexity_type == 'NewBoundMcAllaster' or prm.complexity_type == 'NewBoundSeeger':
            ...
            meta_complex_term = torch.sqrt(
                (hyper_kl + math.log(2 * n_train_tasks / delta)) / (2 * (n_train_tasks - 1))
            )
        ...
        elif prm.complexity_type == 'PAC_Bayes_quad' or prm.complexity_type == 'PAC_Bayes_lambda':
            delta = prm.delta
            meta_complex_term = torch.sqrt(
                (hyper_kl + math.log(2 * n_train_tasks / delta)) / (2 * (n_train_tasks - 1))
            )

        elif prm.complexity_type == 'PAC_Bayes_variational_role' or prm.complexity_type == 'PAC_Bayes_variational_kl':
            delta = prm.delta
            meta_complex_term = torch.sqrt(
                (hyper_kl + math.log(2 * n_train_tasks / delta)) / (2 * (n_train_tasks - 1))
            )
...
def get_bayes_task_objective(prm, prior_model, post_model, n_samples, empirical_loss, hyper_kl=0, n_train_tasks=1, noised_prior=True):
    ...
    if complexity_type == 'NoComplexity':
        ...
        complex_term = torch.sqrt( (hyper_kl + tot_kld + math.log(2*n_samples*n_train_tasks / delta)) / (2*( n_samples - 1 )))
    ...
    #******************************************** New PAC Bayes bound test ********************************************#
        ...
        quad_eps = (1 / (2 * n_samples)) * (tot_kld + hyper_kl + math.log(4 * n_train_tasks * math.sqrt(n_samples) / delta))
        ...
        via_eps = 1 / n_samples * (tot_kld + hyper_kl + math.log(4 * n_train_tasks * math.sqrt(n_samples) / delta))

        complex_term_quad = via_eps + torch.sqrt(via_eps * (via_eps + 2 * empirical_loss))
