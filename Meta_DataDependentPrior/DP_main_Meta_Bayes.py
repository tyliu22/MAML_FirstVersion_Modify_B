...
from Meta_DataDependentPrior import DP_meta_test_Bayes, DP_train_data_prior_learning, DP_test_data_prior_learning, DP_meta_train_Bayes_finite_tasks, DP_meta_train_Bayes_infinite_tasks
from Data_Path import get_data_path
from Models.stochastic_models import get_model
from Utils.DP_data_gen import Task_Generator
from Utils.common import save_model_state, load_model_state, create_result_dir, set_random_seed, write_to_log, save_run_data
from Meta_DataDependentPrior.DP_Analyze_Prior import run_prior_analysis
...
# -------------------------------------------------------------------------------------------
parser.add_argument('--data_prior_ratio', type=float, help='the proportion of prior dataset',
                    default=0.2)

parser.add_argument('--data_prior_batch_size', type=int, help='the proportion of prior dataset',
                    default=128)

parser.add_argument('--data_prior_test_epochs', type=int, help='test epochs in data dependent phase',
                    default=20)

parser.add_argument('--data_prior_train_epochs', type=int, help='train epochs in data dependent phase',
                    default=20)
...
if prm.mode == 'MetaTrain':
    ...
    if n_train_tasks:
        ...
        write_to_log('----- Meta-Training: before training random prior model analysis parameter', prm)
        mean_list, std_list = run_prior_analysis(prior_model, showPlt=False)
        write_to_log('----- Analyze model: mean_list{}, std_list: {}'
                     .format(mean_list, std_list), prm)

        # data dependent prior to learn a prior model
        data_prior_model = DP_train_data_prior_learning.run_prior_learning(train_data_loaders, prm, prior_model)
        write_to_log('----- Meta-Training: After data dependent prior model analysis parameter', prm)
        mean_list, std_list = run_prior_analysis(data_prior_model, showPlt=False)
        write_to_log('----- Meta-Training: After data dependent prior model : mean_list{}, std_list: {}'
                     .format(mean_list, std_list), prm)

        # Meta-training to learn prior:
        trained_prior_model = DP_meta_train_Bayes_finite_tasks.run_meta_learning(train_data_loaders, prm, data_prior_model)
        write_to_log('----- Meta-Training: After training prior model analysis parameter', prm)
        mean_list, std_list = run_prior_analysis(trained_prior_model, showPlt=False)
        write_to_log('----- Meta-Training: After training prior model : mean_list{}, std_list: {}'
                     .format(mean_list, std_list), prm)
        ...
        trained_prior_model = DP_meta_train_Bayes_infinite_tasks.run_meta_learning(task_generator, prm)
...
for i_task in range(n_test_tasks):
    ...
    # data dependent prior to learn a prior model
    data_prior_model = DP_test_data_prior_learning.run_prior_learning(task_data, prm, trained_prior_model)
    if i_task==1:
        write_to_log('----- Meta-Testing: data dependent prior model analysis parameter', prm)
        mean_list, std_list = run_prior_analysis(data_prior_model, showPlt=False)
        write_to_log('----- Meta-Testing: data dependent prior model : mean_list{}, std_list: {}'
                     .format(mean_list, std_list), prm)

    test_err_vec[i_task], test_loss_vec[i_task], test_bound_vec[i_task], post_model = DP_meta_test_Bayes.run_learning(task_data, data_prior_model, prm, init_from_prior, verbose=0)

    if i_task==1:
        write_to_log('----- Meta-Testing: finally task based post model analysis parameter', prm)
        mean_list, std_list = run_prior_analysis(post_model, showPlt=False)
        write_to_log('----- Meta-Testing: finally task based post model : mean_list{}, std_list: {}'
                     .format(mean_list, std_list), prm)

# save result
save_run_data(prm, {'test_err_vec': test_err_vec,"test_loss_vec": test_loss_vec,'test_bound_vec': test_bound_vec})
# save_run_data(prm, {})
# save_run_data(prm, {})
...
write_to_log('----- Meta-Testing - Avg test err: {:.4}%, STD: {:.4}%, Confidence interval: {:.4}%'
             .format(100 * test_err_vec.mean(), 100 * test_err_vec.std(), 1.96*test_err_vec.std()/np.sqrt(n_test_tasks) ), prm)
write_to_log('----- Meta-Testing - Avg test loss: {:.4}, STD: {:.4}, Confidence interval: {:.4}'
             .format(test_loss_vec.mean(), test_loss_vec.std(), 1.96*test_loss_vec.std()/np.sqrt(n_test_tasks) ), prm)
write_to_log('----- Meta-Testing - Avg test bound: {:.4}, STD: {:.4}, Confidence interval: {:.4}'
             .format(test_bound_vec.mean(), test_bound_vec.std(), 1.96*test_bound_vec.std()/np.sqrt(n_test_tasks) ), prm)
